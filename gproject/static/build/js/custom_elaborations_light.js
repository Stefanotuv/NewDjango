...
function issues_summary(headers_check,rows_check){
    ...
    for (var i =0; i<headers_check['not_included'].length;i++){
        ...
        tr_c.onclick = function(){ window.alert("download template file to add the missing column");};
    ...
    td_r_2.innerHTML = "# of row (s): " + parseInt(rows_check.length);
    ...
    for(var k=0; k<rows_check.length;k++){
        ...
        if (str !== ""){
            ...
            row_str = row_str +"[row: " + parseInt(k) + "] ";
            // div = document.createElement("div");
            // div.className = "tooltip";
            par = document.createElement("a");
            par.setAttribute("href","#");
            par.setAttribute("data-toggle","tooltip");
            par.setAttribute("title",str);
            // par.className ="tooltiptext";
            // div.append(par);
            par.innerHTML = row_str;
            par.onclick = function(){
            debugger;
            index= parseInt(this.innerText.split(" ")[1]) ;


            // ------------ for the moal ------------
            record_num.innerText = index;
            ...
            document.getElementById("modal-step-"+ (index)).style.display = "block";
